...
class IGB260M(object):
    ...
    def paper_feat(self) -> np.ndarray:
        ...
        if self.size == "large" or self.size == "full":
            if is_this_machine_bafs() and self.size == "full":
                path = get_bafs_path(
            ...
            if is_this_machine_bafs() and self.size == "full":
                path = get_bafs_path(
                    self.size, "processed", "paper", "node_feat.npy"
                )
            else:
                path = osp.join(
    ...
    def paper_label(self) -> np.ndarray:
        ...
        if self.size == "large" or self.size == "full":
            ...
            if self.num_classes == 19:
                if is_this_machine_bafs() and self.size == "full":
                    path = get_bafs_path(
                        self.size, "processed", "paper", "node_label_19.npy"
                    )
                else:
                    path = osp.join(
                        self.dir,
                ...
                if is_this_machine_bafs() and self.size == "full":
                    path = get_bafs_path(
                        self.size, "processed", "paper", "node_label_2K.npy"
                    )
                else:
                    path = osp.join(
                        self.dir,
    ...
    def paper_edge(self) -> np.ndarray:
        if is_this_machine_bafs() and self.size == "full":
            path = get_bafs_path(
                self.size, "processed", "paper__cites__paper", "edge_index.npy"
            )
        else:
            path = osp.join(
                self.dir,
...
class IGB260MDGLDataset(DGLDataset):
    ...
    def process(self):
        dataset = IGB260M(
            ...
            dummy_feats=self.args.dummy_feats,
        )
    ...
    def process(self):
        if self.args.in_memory:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                paper_paper_edges_path = get_bafs_path(
                ...
                paper_paper_edges_path = osp.join(
                    self.dir,
                    self.args.dataset_size,
                    "processed",
                    "paper__cites__paper",
            ...
            paper_paper_edges = torch.from_numpy(
                np.load(paper_paper_edges_path)
            )

            if is_this_machine_bafs() and self.args.dataset_size == "full":
                author_paper_edges_path = get_bafs_path(
                    self.args.dataset_size,
                    "processed",
                ...
                author_paper_edges_path = osp.join(
                    self.dir,
                    self.args.dataset_size,
                    "processed",
                    "paper__written_by__author",
            ...
            author_paper_edges = torch.from_numpy(
                np.load(author_paper_edges_path)
            )

            if is_this_machine_bafs() and self.args.dataset_size == "full":
                affiliation_author_edges_path = get_bafs_path(
                    self.args.dataset_size,
                    "processed",
                ...
                affiliation_author_edges_path = osp.join(
                    self.dir,
                    self.args.dataset_size,
                    "processed",
                    "author__affiliated_to__institute",
            ...
            affiliation_author_edges = torch.from_numpy(
                np.load(affiliation_author_edges_path)
            )

            if is_this_machine_bafs() and self.args.dataset_size == "full":
                paper_fos_edges_path = get_bafs_path(
                    self.args.dataset_size,
                    "processed",
                    "paper__topic__fos",
                    "edge_index.npy",
            ...
            paper_paper_edges = torch.from_numpy(
                np.load(paper_paper_edges_path, mmap_mode="r")
            )
            ...
            author_paper_edges = torch.from_numpy(
                np.load(author_paper_edges_path, mmap_mode="r")
            )
            ...
            affiliation_author_edges = torch.from_numpy(
                np.load(affiliation_author_edges_path, mmap_mode="r")
            )
            ...
            paper_fos_edges = torch.from_numpy(
                np.load(paper_fos_edges_path, mmap_mode="r")
            )
        ...
        if self.args.all_in_edges:
            graph_data = {
                ...
                ("institute", "affiliated_to", "author"): (
        ...
        if self.args.in_memory:
            ...
            paper_node_features = torch.from_numpy(
                np.load(paper_node_features_path)
            )
            if self.args.num_classes == 19:
                if is_this_machine_bafs() and self.args.dataset_size == "full":
                ...
                paper_node_labels = torch.from_numpy(
                    np.load(paper_node_labels_path)
                ).to(torch.long)
            else:
                if is_this_machine_bafs() and self.args.dataset_size == "full":
                    paper_node_labels_path = get_bafs_path(
            ...
            paper_node_features = torch.from_numpy(
                np.load(paper_node_features_path, mmap_mode="r")
            )
            if self.args.num_classes == 19:
                if is_this_machine_bafs() and self.args.dataset_size == "full":
                    paper_node_labels_path = get_bafs_path(
                ...
                paper_node_labels = torch.from_numpy(
                    np.load(paper_node_labels_path, mmap_mode="r")
                ).to(torch.long)
            else:
                if is_this_machine_bafs() and self.args.dataset_size == "full":
                    paper_node_labels_path = get_bafs_path(
        ...
        if self.args.in_memory:
            ...
            author_node_features = torch.from_numpy(
                np.load(author_node_features_path)
            )
        else:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                author_node_features_path = get_bafs_path(
            ...
            author_node_features = torch.from_numpy(
                np.load(author_node_features_path, mmap_mode="r")
            )
        ...
        if self.args.in_memory:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                institute_node_features_path = get_bafs_path(
                    self.args.dataset_size,
                    "processed",
                    "institute",
                    "node_feat.npy",
                ...
                institute_node_features_path = osp.join(
                    self.dir,
                    self.args.dataset_size,
                    "processed",
                    "institute",
            ...
            institute_node_features = torch.from_numpy(
                np.load(institute_node_features_path)
            )
        else:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                institute_node_features_path = get_bafs_path(
            ...
            institute_node_features = torch.from_numpy(
                np.load(institute_node_features_path, mmap_mode="r")
            )
        ...
        if self.args.in_memory:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                fos_node_features_path = get_bafs_path(
                    self.args.dataset_size, "processed", "fos", "node_feat.npy"
                )
            else:
                fos_node_features_path = osp.join(
                    self.dir,
                    self.args.dataset_size,
                    "processed",
                    "fos",
            ...
            fos_node_features = torch.from_numpy(
                np.load(fos_node_features_path)
            )
        else:
            if is_this_machine_bafs() and self.args.dataset_size == "full":
                fos_node_features_path = get_bafs_path(
            ...
            fos_node_features = torch.from_numpy(
                np.load(fos_node_features_path, mmap_mode="r")
            )
...
class IGBHeteroDGLDatasetMassive(DGLDataset):
    ...
    def process(self):
        ...
        if self.args.dummy_feats:
            ...
            if is_this_machine_bafs() and self.args.size == "full":
                fos_node_features_path = get_bafs_path(
                    self.args.dataset_size, "processed", "fos", "node_feat.npy"
                )
            else:
        ...
        num_nodes_dict = {
            ...
            "institute": len(institute_node_features),
            "fos": len(fos_node_features),
        }
        ...
        if self.args.load_homo_graph:
            ...
            self.graph = construct_homogeneous(
                graph_data,
                [
                    ("author", "affiliated_to", "institute"),
                    ("paper", "cites", "paper"),
            ...
            self.graph.nodes["institute"].data[
                "feat"
            ] = institute_node_features
            self.graph.num_institute_nodes = institute_node_features.shape[0]
