...
def split_single_line(line: str) -> List[str]:
    ...
    line_patterns = [
        ".*? \d*: Part \d* \| Epoch \d* \| Step \d* \| Loss [\d.]* \| Train Acc [\d.]* \| Speed \(samples\/sec\) [\d.]* \| GPU [\d.]* MB \| Mean step time [\d.]* s",
...
def extract_timing(
    filename: str, epoch_to_collect=1, num_steps_to_collect_at_the_end=100
):
    """
    ...
    part_unknown_epoch_step_logs: dict[tuple[str, int], dict] = (
        dict()
    )  # The epoch and step is unknown when the line is [DistDataLoader _request_nest_batch]. We need to wait for the next line to get the epoch.
    ...
    for line in lines:
        try:
            if line.find("_request_nest_batch]") != -1:
                ...
                part_unknown_epoch_step_logs[(node, partition_id)][
                    "sampling_time"
                ] = sampling_time
                part_unknown_epoch_step_logs[(node, partition_id)][
                    "aggregation_time"
                ...
                if epoch_idx not in part_epoch_step_logs[(node, partition_id)]:
                    part_epoch_step_logs[(node, partition_id)][
                        epoch_idx
                    ] = dict()
                if (
                    step_idx
                    not in part_epoch_step_logs[(node, partition_id)][
                        epoch_idx
                    ]
                ):
                    part_epoch_step_logs[(node, partition_id)][epoch_idx][
                        step_idx
                    ] = dict()
                part_epoch_step_logs[(node, partition_id)][epoch_idx][
                    step_idx
                ]["sampling_time"] = part_unknown_epoch_step_logs[
                    (node, partition_id)
                ][
                    "sampling_time"
                ]
                part_epoch_step_logs[(node, partition_id)][epoch_idx][
                    step_idx
                ]["aggregation_time"] = part_unknown_epoch_step_logs[
                    (node, partition_id)
                ][
                    "aggregation_time"
                ]
                part_epoch_step_logs[(node, partition_id)][epoch_idx][
                    step_idx
                ]["sample_aggregation_time"] = sample_aggregation_time
                part_epoch_step_logs[(node, partition_id)][epoch_idx][
                    step_idx
    ...
    for node, partition_id in part_epoch_step_logs:
        last_step_idx = max(
            part_epoch_step_logs[(node, partition_id)][epoch_to_collect].keys()
        )
        sampling_times = []
        aggregation_times = []
        ...
        movement_times = []
        train_times = []
        for idx_step in range(
            last_step_idx, last_step_idx - num_steps_to_collect_at_the_end, -1
        ):
            try:
                sampling_times.append(
                    part_epoch_step_logs[(node, partition_id)][
                        epoch_to_collect
                    ][idx_step]["sampling_time"]
                )
            except Exception as e:
                print(
                    f"[Unknown key] sampling_time:",
                    (node, partition_id),
                    epoch_to_collect,
                    idx_step,
                )
            try:
                aggregation_times.append(
                    part_epoch_step_logs[(node, partition_id)][
                        epoch_to_collect
                    ][idx_step]["aggregation_time"]
                )
            except:
                print(
                    f"[Unknown key] aggregation_time:",
                    (node, partition_id),
                    epoch_to_collect,
                    idx_step,
                )
            try:
                sample_aggregation_times.append(
                    part_epoch_step_logs[(node, partition_id)][
                        epoch_to_collect
                    ][idx_step]["sample_aggregation_time"]
                )
            except:
                print(
                    f"[Unknown key] sample_aggregation_time:",
                    (node, partition_id),
                    epoch_to_collect,
                    idx_step,
                )
            try:
                movement_times.append(
                    part_epoch_step_logs[(node, partition_id)][
                        epoch_to_collect
                    ][idx_step]["movement_time"]
                )
            except:
                print(
                    f"[Unknown key] movement_time:",
                    (node, partition_id),
                    epoch_to_collect,
                    idx_step,
                )
            try:
                ...
                print(
                    f"[Unknown key] train_time:",
                    (node, partition_id),
                    epoch_to_collect,
                    idx_step,
                )
        avg_metrics[(node, partition_id)] = dict()
        avg_metrics[(node, partition_id)]["sampling_time"] = sum(
            sampling_times
        ) / len(sampling_times)
        avg_metrics[(node, partition_id)]["aggregation_time"] = sum(
            aggregation_times
        ) / len(aggregation_times)
        avg_metrics[(node, partition_id)]["sample_aggregation_time"] = sum(
            sample_aggregation_times
        ) / len(sample_aggregation_times)
        avg_metrics[(node, partition_id)]["movement_time"] = sum(
            movement_times
        ) / len(movement_times)
        avg_metrics[(node, partition_id)]["train_time"] = sum(
            train_times
        ) / len(train_times)
        print(
            f"Node: {node}, Partition: {partition_id}, Epoch: {epoch_to_collect}, Avg. Sampling Time: {avg_metrics[(node, partition_id)]['sampling_time']} sec, Avg. Aggregation Time: {avg_metrics[(node, partition_id)]['aggregation_time']} sec, Avg. Sample + Aggregation Time: {avg_metrics[(node, partition_id)]['sample_aggregation_time'] } sec, Movement Time: {avg_metrics[(node, partition_id)]['movement_time'] } sec, Avg. Train Time: {avg_metrics[(node, partition_id)]['train_time']} sec"
        )
