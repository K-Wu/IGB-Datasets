# Launcher for use in slurm script
...
def get_ip_address_to_node_mapping() -> Dict[str, str]:
    ...
    for node in node_lists:
        ip = subprocess.check_output("getent ahostsv4 "+ node+ " | grep STREAM | head -n 1  | awk '{ print $1 }'", shell=True, stderr=subprocess.STDOUT).decode("utf-8").strip()
        results[ip] = node
    return results
